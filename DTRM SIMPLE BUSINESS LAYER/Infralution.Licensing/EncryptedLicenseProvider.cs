...
namespace Infralution.Licensing
{
    ...
    class EncryptedLicenseProvider : LicenseProvider
	{
        ...
        /// <param name="keyStrength">The strength of the key used for verification</param>
        ...
        /// </remarks>
        public string GenerateLicenseParameters(string password, int keyStrength)
        {
            ...
            byte[] designKey = PadKey(GetDesignKey(password, keyStrength)); 
            byte[] designSignature = rsa.SignData(designKey, new SHA1CryptoServiceProvider());

            // encrypt the design key using itself to produce the runtime key
            //
            byte[] encryptionKey = new byte[ArraySize(8)];
            Array.Copy(designKey, 0, encryptionKey, 0, 7);
            ...
            byte[] runtimeKey = new byte[ArraySize(keyStrength)];
            Array.Copy(encKey, 0, runtimeKey, 0, keyStrength);
            runtimeKey = PadKey(runtimeKey);
            ...
            writer.WriteElementString("KeyStrength", keyStrength.ToString());
        ...
        /// </remarks>
        public virtual string GenerateKey(int keyStrength, string password, string productInfo, Int32 serialNo)
        {
            if (keyStrength < 7) throw new ArgumentOutOfRangeException("keyStrength", "KeyStrength must be 7 or greater");
            if (password == null) throw new ArgumentNullException("password");
            ...
            byte[] designKey = GetDesignKey(password, keyStrength); 
            ...
            // Encrypt the payload. The key used for encrypting the payload is just the first 7 bytes of the design key
            //
            byte[] encryptionKey = new byte[ArraySize(8)];
            Array.Copy(designKey, 0, encryptionKey, 0, 7);
            DESCryptoServiceProvider des = new DESCryptoServiceProvider();
            ...
            //
            byte[] data = new byte[ArraySize(designKey.Length + encPayload.Length)];

            // For key strengths greater than 8 we swap the order of the payload and design key
            // This ensures that the first 8 bytes do not always end up hex encoded the same
            //
            if (keyStrength < 8)
            {
                designKey.CopyTo(data, 0);
                encPayload.CopyTo(data, designKey.Length);
            }
            else
            {
                encPayload.CopyTo(data, 0);
                designKey.CopyTo(data, encPayload.Length);
            }
        ...
        /// <param name="keyStrength">The strength of the key</param>
        /// <returns>The license data if successful or null if not</returns>
        internal EncryptedLicense ExtractLicense(string licenseKey, string password, int keyStrength)
        {
            try
            {
                ...
                // extract the design key and encrypted product data - note that the encryption
                // key has only 7 significant bytes 
                //
                byte[] designKey = new byte[ArraySize(keyStrength)];
                byte[] encPayload = new byte[ArraySize(data.Length - keyStrength)];

                // for key strengths greater than 8 the order of payload and key is swapped
                //
                if (keyStrength < 8)
                {
                    Array.Copy(data, 0, designKey, 0, keyStrength);
                    Array.Copy(data, keyStrength, encPayload, 0, encPayload.Length);
                }
                else
                {
                    Array.Copy(data, 0, encPayload, 0, encPayload.Length);
                    Array.Copy(data, encPayload.Length, designKey, 0, keyStrength);
                }

                // The key used for encrypting data is just the first 7 bytes of the design key
                //
                byte[] encryptionKey = new byte[ArraySize(8)];
                Array.Copy(designKey, 0, encryptionKey, 0, 7);
                ...
                //
                byte[] requiredDesignKey = GetDesignKey(password, keyStrength);
                if (!ArrayEqual(designKey, requiredDesignKey))
                {
                    return null;
                }
        ...
        /// <summary>
        /// Generate the key used to verify and decrypt the license
        /// </summary>
        ...
        /// <returns>The key used to verify and decrypt the license</returns> 
        private static byte[] GetDesignKey(string password, int keyStrength)
        {
        ...
        /// <summary>
        /// Pad the given design/runtime key if required.
        /// </summary>
        /// <param name="key">The design or runtime key for the license</param>
        /// <returns>The padded key</returns> 
        /// <remarks>
        /// This function is required for backward compatibility with 7 byte design keys which were
        ...
        protected virtual EncryptedLicense LoadLicense(LicenseContext context, Type type, string licenseKey)
        {
            ...
            try
            {
                ...
                // extract the design key and encrypted product data 
                //
                byte[] designKey = new byte[ArraySize(_keyStrength)];
                ...
                if (_keyStrength < 8)
                {
                    Array.Copy(data, 0, designKey, 0, _keyStrength);
                    Array.Copy(data, _keyStrength, encPayload, 0, encPayload.Length);
                }
                else
                {
                    Array.Copy(data, 0, encPayload, 0, encPayload.Length);
                    Array.Copy(data, encPayload.Length, designKey, 0, _keyStrength);
                }

                // the key used to encrypt payload is just the first 7 bytes of the design key
                //
                byte[] encryptionKey = new byte[ArraySize(8)];
                Array.Copy(designKey, 0, encryptionKey, 0, 7);
                ...
                byte[] paddedDesignKey = PadKey(designKey);
                if (context.UsageMode == LicenseUsageMode.Designtime)
                {

                    // if design time license requested then the license MUST be a design license
                    //
                    if (!rsa.VerifyData(paddedDesignKey, new SHA1CryptoServiceProvider(), _designSignature)) return null;
                }
                else
                {
                    ...
                    //
                    if (!rsa.VerifyData(paddedDesignKey, new SHA1CryptoServiceProvider(), _runtimeSignature))
                    {
                        if (!rsa.VerifyData(paddedDesignKey, new SHA1CryptoServiceProvider(), _designSignature)) return null;
                    }
                }
                ...
                {
                    // create the runtime password by encrypting the design key
                    //
                    byte[] encKey = des.CreateEncryptor().TransformFinalBlock(paddedDesignKey, 0, paddedDesignKey.Length);
                    byte[] runtimeKey = new byte[ArraySize(_keyStrength)];
                    Array.Copy(encKey, 0, runtimeKey, 0, _keyStrength);

                    // encrypt the payload using the runtime key
                    //
                    byte[] runtimeEncryptionKey = new byte[ArraySize(8)];
                    Array.Copy(runtimeKey, 0, runtimeEncryptionKey, 0,  7);
                    des.IV = runtimeEncryptionKey;
                    encPayload = des.CreateEncryptor().TransformFinalBlock(payload, 0, payload.Length);
      
                    // Combine the runtime key and encrypted payload 
                    //
                    data = new byte[ArraySize(runtimeKey.Length + encPayload.Length)];
                    ...
                    if (_keyStrength < 8)
                    {
                        ...
                        encPayload.CopyTo(data, runtimeKey.Length);
                    }
                    else
                    {
                        encPayload.CopyTo(data, 0);
                        runtimeKey.CopyTo(data, encPayload.Length);
                    }
