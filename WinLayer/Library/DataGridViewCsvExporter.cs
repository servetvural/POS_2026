...
public class DataGridViewCsvExporter
{
    ...
    public bool Generate(bool blnIncludeHeaders, bool blnVisibleCellsOnly, string columnseperator)
    {
        if (dgv != null)
        {
    ...
    {
        return Generate(blnIncludeHeaders, blnVisibleCellsOnly, ",");
    }
    ...
    public List<DataGridViewFormatItem> GetDataGridViewColumnWidths(DataGridView dgv, bool blnVisibleColumnsOnly)
    {
        List<DataGridViewFormatItem> columnWidths = new List<DataGridViewFormatItem>();
        //string formatstring = "";
        int count = 0;
        int alignment = 0;
        for (int i = 0; i < dgv.Columns.Count; i++)
        {
            if (blnVisibleColumnsOnly)
            {
                if (dgv.Columns[i].Visible && dgv.Columns[i].CellType == typeof(DataGridViewTextBoxCell))
                {
                    alignment = dgv.Columns[i].DefaultCellStyle.Alignment == DataGridViewContentAlignment.MiddleLeft ? -1 : 1;
                    columnWidths.Add(new DataGridViewFormatItem(i, (int)((dgv.Columns[i].Width * alignment)), dgv.Columns[i].HeaderText));
                    //formatstring += "{" + count++ + ":D" + dgv.Columns[i].Width + "}";
                }
            } else
                columnWidths.Add(new DataGridViewFormatItem(i, dgv.Columns[i].Width));
        }
        return columnWidths;
    }

    public bool GenerateAsTabularDynamicColumnWidth(bool blnIncludeHeaders, bool blnVisibleCellsOnly)
    {
        if (dgv != null)
        {
            List<DataGridViewFormatItem> columnWidth = GetDataGridViewColumnWidths(dgv, blnVisibleCellsOnly);
            ...
            if (blnIncludeHeaders)
            {
                ...
                for (int i = 0; i < dgv.Columns.Count; i++)
                {
                    if (blnVisibleCellsOnly && !dgv.Columns[i].Visible)
                        continue;
                    string val = dgv.Columns[i].HeaderText;
                    int len = Math.Abs(columnWidth[columns].width);
                    val = val.Length > len ? val.Substring(0, len - 1) : val;
                    headercharcount += len;
                    csvText += string.Format("{0," + columnWidth[columns].width + "}", val); //   .PadRight(Math.Abs(columnWidth[columns]));
                    columns++;
                }
            }
            NewLine();
    ...
    public bool ExportToFile()
    {
        ...
        // dlg.FileName = "DTRMSimple Database Backup";
